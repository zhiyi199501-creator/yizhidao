...
`AI_MODE=mock` 时走规则解读（不耗 token）。`OPENAI_MAX_TOKENS` 默认 2500。口播原文不进 prompt。解读机制见 [`docs/ai-reading.md`](../docs/ai-reading.md)；App 接口合同见 `docs/backend-min-spec.md`（`/v1/admin/*` 不在该合同内）。单测：`.venv/bin/python -m unittest`。抽检同一套夹具：CLI `scripts/eval_ai_reading.py`（`--dry-run` 不调模型；live 写 `backend/.eval/`）或后台「抽检」页（不写用量事件）。AI 限流：登录用户、解读与追问合计自然日未购 3 次 / 买断 30 次（UTC+8）、间隔 8 秒；`AI_RATE_*` / `IAP_*` 见 `.env.example`。
