...
- **我的**：登录为 iOS **Apple** / Android **Google**（主按钮）+ 邮箱验证码（子页）；无微信、登录页无短信。资料编辑、**基础入门**、**六十四卦**（详情卡片彖辞/大象；经文可点 IMA 讲解；页底证释来源）、**四传**（系辞/说卦/序卦/杂卦；页底证释来源）、**案例**（按卦浏览，打开拉取 `GET /v1/cases`，离线用包内底稿）、**意见反馈**、**检查更新**、设置（回收站，清空需确认；退出登录；**注销账号**）。繁简跟系统语言，无应用内开关。无按键音效。
