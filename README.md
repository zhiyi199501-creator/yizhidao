...
- **起卦（iOS）**：页上只选法门（输入三数 / 时间起卦 / 金钱起卦），无所问、无取数表单。点开始后走全屏仪式：静心 → 告神（所问必填）→ 取数（三数一个一个落 / 金钱一爻一爻摇 / 时间取敬告那一刻）→ 揭卦。规矩与五条礼仪在「基础入门」
- **起卦（Android）**：仍是旧页（所问必填、页内取数、可折叠「起卦礼仪」），仪式盖层未跟
- **数字起卦 · 时间**：默认十二时辰；可开「公历取数」（公历月日 + 1–24 时）。停在「此刻」时取数以敬告那一刻为准，手选过才钉死时刻
- **金钱起卦（iOS）**：摇手机或轻点铜钱，一次一爻，满六自动成卦；长按手选四象；无「一键摇满」。上爻在上、初爻在下
...
- **我的**：登录为 iOS **Apple** / Android **Google**（主按钮）+ 邮箱验证码（子页）；无微信、登录页无短信。资料编辑、**基础入门**、**六十四卦**（详情卡片彖辞/大象；经文可点 IMA 讲解）、**四传**（系辞/说卦/序卦/杂卦）、**案例**（按卦浏览，打开拉取 `GET /v1/cases`，离线用包内底稿）、**意见反馈**、**检查更新**、设置（按键音效、回收站，清空需确认；退出登录；**注销账号**）。繁简跟系统语言，无应用内开关。
