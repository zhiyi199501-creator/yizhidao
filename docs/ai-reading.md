...
结果页悬浮 **问**：该占已有问答则直接打开；没有则自动 `POST /v1/ai/analyze`（需登录）。页标题「问答」。卡片：事情背景 / 当下 / 方向 / 建议 / 可以接着问。须防只一条，并入建议（条目前加「须防：」），接口仍单独返回 `risks`。App 起卦所问必填；接口仍接受空所问（旧记录），空时「可以接着问」固定为「我的事业会如何？」「我的感情会如何？」。点短问即发出（不填入输入框），或自写后发 → `POST /v1/ai/followup` → 回复 + 这一轮建议；最新一轮仍给「可以接着问」（须是用户口吻，用「我」）。初次的短问在已有追问后藏掉。问答自动保存到本地「问答」Tab（一占一条）；追问成功会更新该条。
