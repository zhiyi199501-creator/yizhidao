...
**现役核验（2026-08-28）**：`GET https://api.yiwanjia.work/health` 200；`/privacy` `/terms` `/support` 200；响应 **HTTP/2**，无 `alt-svc`。新加坡机跑 `docker compose`（`backend-caddy-1` + `backend-api-1`），**不是** `docker-compose.prod.yml`。AI 扩卡已合 `main`（PR #12），镜像未重建，解读仍是三字段。法律英文本（`?lang=en`）2026-08-30 抽检仍出中文，须发含 `*.en.html` 的镜像后才现役。
