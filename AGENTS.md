...
**Android（本分支 `feature/classic-yijing-me`）**：引擎对拍 iOS 单测；Compose 四 Tab、历史回收站、繁简切换、按键音效。短信登录、资料编辑、AI 解读＋追问＋保存的AI解读（已保存后追问自动更新；重新解读→「重新保存」；列表左滑删除）、案例 `GET /v1/cases` 热更新、六十四卦详情（卦象 50% 缩放；卡片标题彖辞/大象）。AI 页打开时隐藏底部 Tab；`windowSoftInputMode=adjustNothing`，追问栏 `imePadding` 贴键盘（edge-to-edge）。Release API：`https://yzh.codedance.work`；Debug 真机 API 在 `android/app/build.gradle.kts` 配 Mac 局域网 IP。未做：微信登录。见 `android/README.md`。
