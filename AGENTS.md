...
- `Features/Cases/`：按卦列表（无时间、无应验徽章）；iOS 打开时 `GET /v1/cases` 拉最新，失败则用包内/缓存；详情为背景 / 所问 / 验证 / 讲师解读 + 本卦之卦
...
- 经文底稿：`Yizhidao/Resources/Hexagrams.json`。编辑根目录 `易经正文编辑表.xlsx`（gitignore）→ `python3 scripts/import_jingwen.py`；导出 `python3 scripts/export_jingwen.py`
...
- `App/`：`AppNavigation`、`AppTheme`、登录与「我的」（多在 `YizhidaoApp.swift`）。「我的」：资料、**AI解读历史**（本地保存的解读）、**易经基础入门** `YijingIntro.json`、**易经六十四卦 / 易经四传** 读 `Hexagrams.json`、设置（回收站，清空需确认；退出登录）
...
**main / 生产（PR #4，2026-08-17 在源站核过）**：双法起卦、礼仪、主看、历史、案例按卦、登录、`GET /v1/me`、AI 解读＋追问＋本地保存。`https://yizhidao.codedance.work` 有 `analyze` / `followup` / `me` / **`GET /v1/cases`（313 条，谦卦 0）**。iOS 打开案例页拉取；失败用包内/缓存。

未做：微信登录、生产短信、App Store、文言讲解层。运维见 `docs/deploy.md`。

**Android（已进 main）**：引擎对拍 iOS 单测；Compose 四 Tab 可用；登录、AI、回收站、案例热更新未接。见 `android/README.md`。
