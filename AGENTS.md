...
**资料同步（本地未提交、未发；2026-08-31）**：头像／昵称进服务端，无邮箱可绑定；头像文件在 `backend/avatars/`（勿提交）。接口见 `docs/backend-min-spec.md`。

**后台白底（本地未提交、未发）**：`admin/` 浅灰底、白卡片、蓝强调。勿改回宣纸。本机 `8080/admin/` 读的是 `admin/dist`。
